...
class Tiket():
    ...
    def saveData(self):
        ...
        try:
            ...
            try:
                ...
                True
            try:
                database.update_row_odp(self,dist)
    ...
    def reviewTiket(self):
        ...
        if(self.latitude!=""):
            ...
            status=False
        if(dist>general.max_dist):
            self.send_message("Jarak yang dikirimkan melebihi {} m.".format(general.max_dist))
        if(status):   
            general.reply_keyboard("Edit Kembali atau Submit?",self.chatId,general.json_aft_review)
        else:
           general.reply_keyboard("Data belum lengkap atau gambar ada yang sama. Silakan lengkapi atau ganti",self.chatId,self.keyboard.json_lvl_1)
       
    def reviewTiketWeb(self):
        self.send_message("Isi tiket")
        self.send_message("noTiket: {}".format(self.noTiket))
        hasil_sql=database.select_review(self)
    ...
    def save_file(self, fileId,path):
        url = general.URL + "getFile?file_id={}".format(fileId)
        js = general.get_json_from_url(url)
        file_path = js["result"]["file_path"]
        url = general.URL_FILE + file_path
    ...
    def sendImage(self,path,caption):
        try:
            url_send = general.URL+"sendPhoto";
            print(url_send)
    ...
    def send_message(self, text):
        url = general.URL + "sendMessage?text={}&chat_id={}".format(text, self.chatId)
        general.get_url(url)
            
    def send_file(self, file_id,text):
        url = general.URL + "sendPhoto?photo={}&chat_id={}&caption={}".format(file_id, self.chatId,text)
        general.get_url(url)
    
    def send_location(self):
        url = general.URL + "sendlocation?chat_id={}&latitude={}&longitude={}".format(self.chatId, self.latitude, self.longitude)
        general.get_url(url)
